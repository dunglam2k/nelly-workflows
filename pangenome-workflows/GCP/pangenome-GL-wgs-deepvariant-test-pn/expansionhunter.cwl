...
arguments:
  ...
  - |
    ...
    ExpansionHunter --reads "$(inputs.aligned_reads.path)" --reference "$(inputs.ref.path)" --variant-catalog "$CATALOG" --sex "$(inputs.sex)" --output-prefix "$(inputs.output_prefix)" --threads $(runtime.cores)
